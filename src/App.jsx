...
export default function App() {
  ...
  const syncRecord = async (table, record) => {
    try {
      ...
      // WORKAROUND: Remove abandonedReason/abandonReason to avoid schema cache bug
      const cleanRecord = { ...record };
      if (table === 'tasks') {
        delete cleanRecord.abandonedReason; // with 'ed'
        delete cleanRecord.abandonReason;   // without 'ed' (used in UI)
      }
      if (table === 'projects') {
        // Keep abandonedDate for Dashboard graph, but remove abandonReason (cache bug)
        delete cleanRecord.abandonReason; // projects use abandonReason (no 'ed')
      }
  ...
  const doPrintVisibleProjects = (includeTasks, includeCompletedTasks) => {
    ...
    if (visibleProjects.length === 0) {
      alert("Aucun projet visible \ud83d\udd25 à imprimer.");
      return;
    }
...
<head>
  <meta charset="UTF-8">
  <title>Projets - Impression Vue</title>
  <style>
    @media print {
      @page { margin: 1.5cm; }
...
`;
    ...
    visibleProjects.forEach(p => {
      ...
      const activeTasks = projectTasks
        .filter(t => t.status !== "Terminé" && t.status !== "Abandonné")
        .sort((a, b) => {
          ...
          <div>
            <div style={{ ...s.sectionTitle, marginBottom: 16 }}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <button style={s.btn("ghost")} onClick={() => printVisibleProjects()}>🖨️ Imprimer vue</button>
